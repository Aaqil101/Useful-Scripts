...
test_time() {
    ...
    if [ $hour_int -ge 18 ] && [ $hour_int -le 23 ]; then
        ...
        expected="Today (still sleeping - early morning)"
        result=$(date +%Y-%m-%d)
    else
...
if [ $# -eq 1 ]; then
    ...
    echo "--- Early Morning (12 AM - 5:59 AM) - Returns TODAY ---"
    ...
    echo "• 12:00 AM - 5:59 AM → TODAY (still sleeping, but already on the new day)"
    echo "• 6:00 AM - 5:59 PM → TODAY (awake, daytime)"
    echo ""
